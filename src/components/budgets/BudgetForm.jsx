...
export default function BudgetForm({ budget, tags, onSave, onCancel }) {
  ...
  useEffect(() => {
    ...
    const initialTagId = budget?.tag_id_base44 || (tags.length > 0 ? tags[0].id : ""); // Changed
    setFormData({
      tag_id_base44: initialTagId, // Changed
      amount: budget?.amount || 0,
      period: budget?.period || "monthly",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onSave({
        ...
        tag_id_base44: formData.tag_id_base44 || null // Ensure null if empty, though already handled by form state
      });
    } catch (error) {
  ...
  const selectedTag = tags.find(t => t.id === formData.tag_id_base44); // Changed
      ...
      <CardContent className="p-8 overflow-y-auto flex-grow">
        <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {filteredTags.map((tag) => (
                          <CommandItem
                            ...
                            onSelect={(currentValue) => {
                              ...
                              handleInputChange("tag_id_base44", currentSelectedTag ? currentSelectedTag.id : ""); // Changed
                              setTagSearchValue(currentSelectedTag ? currentSelectedTag.name : "");
                              setTagPopoverOpen(false);
                            }}
                          >
                            <Check
                              className={`mr-2 h-4 w-4 ${formData.tag_id_base44 === tag.id ? "opacity-100" : "opacity-0"}`} // Changed
                            />
                            {tag.name}
                          </CommandItem>
                        ))}
          ...
          <div className="flex justify-end gap-4 pt-6 border-t sticky bottom-0 bg-white py-6 z-10">
            ...
            <Button
              type="submit"
              disabled={isLoading || !formData.tag_id_base44}
